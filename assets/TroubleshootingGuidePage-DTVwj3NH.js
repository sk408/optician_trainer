import{r as u,j as e,B as o,C as S,P as h,T as n,b as g,c as f,e as y,D as j,a as c,L as l}from"./index-n2vOrYu9.js";import{A as T,a as k,E as q,b as I}from"./ExpandMore-DQiwWpus.js";import{T as D,a as v}from"./Tabs-D5iwso7h.js";import{G as w}from"./Grid-jA4lGWB_.js";import"./Collapse-_racRJxa.js";const M={title:"Common Troubleshooting Guide for Eyewear",description:"Practical guide to diagnosing and resolving common issues with glasses and contact lenses",sections:[{title:"Troubleshooting Glasses Fit Issues",content:`
        Proper glasses fit is essential for comfort and optimal vision. Here's how to diagnose and fix common fit problems:

        1. Glasses Sliding Down
        - Cause: Nose pads too wide or frame too heavy
        - Solution: Adjust nose pads inward or tighten temple arms
        - Additional Fix: Consider adding silicone nose grips for plastic frames
        - When to Refer: If weight is the primary issue, suggest a lighter frame material

        2. Pinching at the Nose
        - Cause: Nose pads too close together
        - Solution: Gently adjust nose pads outward
        - Check For: Redness or indentation marks on the nose bridge
        - Prevention: Select frames with adjustable nose pads for patients with narrow bridges

        3. Pressure Behind the Ears
        - Cause: Temple arms too tight or incorrect angle
        - Solution: Adjust temple arms outward slightly and ensure proper downward angle
        - Technique: Heat temple tips for easier adjustment with acetate frames
        - Evaluation: Ensure 45° angle at the bend for proper weight distribution

        4. Uneven Frame
        - Cause: Uneven adjustment or facial asymmetry
        - Solution: Adjust frame alignment using proper optical tools
        - Assessment: Look for level lens heights and even temple positioning
        - Advanced Technique: Use frame warmer for more precise adjustments
      `,keyPoints:["Most fit issues can be resolved with proper adjustment techniques","Always verify adjustments by having the patient wear glasses for at least 5 minutes","Document all adjustments made for future reference"]},{title:"Vision Problems With New Glasses",content:`
        When patients report vision issues with new glasses, follow this troubleshooting path:

        1. Blurry Vision
        - First Check: Verify prescription against lab order
        - Measurement Check: Confirm PD and OC heights are correct
        - Physical Inspection: Look for lens warpage or fabrication errors
        - Adaptation Period: Explain normal adaptation time (1-2 weeks for progressive lenses)

        2. Double Vision
        - Potential Causes: Incorrect PD, vertical imbalance, or prismatic effect
        - Check For: Lens centration and vertical alignment
        - Tests: Cover test to check for underlying binocular vision issues
        - Solution: Adjust lens position or remake with corrected measurements

        3. Distortion with Progressive Lenses
        - Common Issues: "Swimming" sensation, peripheral distortion
        - Frame Check: Ensure frame is properly adjusted and not sitting too low/high
        - Measurement Verification: Confirm fitting cross alignment with pupil
        - Adaptation Tips: Teach proper head positioning and suggest gradual wear increase

        4. Headaches or Eye Strain
        - First Action: Check for excessive lens tilt or wrap angle
        - Common Causes: Incorrect cylinder axis, PD errors, or excessive prismatic effect
        - Solution Path: Verify all measurements and make minor adjustments before considering remake
        - Documentation: Track time of day and specific activities that trigger symptoms
      `,keyPoints:["Always verify prescription and measurements before assuming fabrication error","Many vision issues resolve with proper adjustment and adaptation time","Document all patient complaints specifically for better troubleshooting"]},{title:"Contact Lens Troubleshooting",content:`
        Contact lens issues require systematic assessment:

        1. Discomfort or Awareness
        - Initial Check: Examine for debris, tears, or inside-out lenses
        - Fit Evaluation: Check for proper movement and centration
        - Solution Issues: Consider preservative sensitivity or solution incompatibility
        - Action Plan: Try different solution, evaluate fit parameters, or switch material

        2. Vision Fluctuation
        - Common Causes: Lens dehydration, deposits, or fit issues
        - Assessment: Evaluate tear film quality and lens wettability
        - Solution: Consider more frequent replacement schedule or different material
        - Education: Discuss proper blinking techniques and environmental factors

        3. Redness and Irritation
        - Immediate Action: Assess for potential corneal abrasion or infection
        - Check For: Tight lens syndrome, solution sensitivity, or overwear
        - Solution Path: Temporary discontinuation and lubricating drops
        - Referral Criteria: When to refer for medical evaluation immediately

        4. Handling Difficulties
        - Assessment: Evaluate dexterity and technique
        - Teaching Tools: Use colored training lenses for visualization
        - Alternative Options: Discuss daily disposables for convenience
        - Support Materials: Provide step-by-step printed guides with images
      `,keyPoints:["Contact lens issues often require evaluation of multiple factors","Patient education is crucial for resolving and preventing problems","Document all troubleshooting steps for continuity of care"]},{title:"Frame Repair and Maintenance",content:`
        Common frame issues and their solutions:

        1. Loose Screws
        - Prevention: Apply screw lock solution during dispensing
        - Repair: Replace with correct size, avoid overtightening
        - Patient Education: Provide small screwdriver and regular tightening schedule
        - Advanced Tip: Use nylon thread for temporary emergency repair

        2. Broken Temple Hinge
        - Assessment: Determine if temple or frame front hinge is damaged
        - Repair Options: Soldering for metal frames, acetone welding for plastic
        - Temporary Fix: Use heat-shrink tubing for emergency repair
        - Replacement: When to order new parts vs. recommend new frame

        3. Nose Pad Replacement
        - Selection: Choose proper size and style matching original
        - Technique: Proper plier use to avoid thread stripping
        - Material Options: Silicone vs. acetate pads for different skin types
        - Hypoallergenic Options: For patients with metal sensitivity

        4. Bridge Repair
        - Common Issues: Broken bridges in plastic frames
        - Repair Technique: Proper pin installation and acetone welding
        - Limitations: When repair will compromise structural integrity
        - Alternative: Discussing frame replacement when repair is not viable
      `,keyPoints:["Quality repairs require proper tools and techniques","Set realistic expectations about durability of repairs","Prevention through proper maintenance should be emphasized"]},{title:"Lens Surface Issues",content:`
        Troubleshooting common lens surface problems:

        1. Scratched Lenses
        - Prevention: Proper case use and cleaning instructions
        - Assessment: Depth and location of scratches
        - Solution: AR coating reapplication for minor scratches
        - Recommendation: Discuss scratch-resistant treatments for future purchases

        2. Coating Peeling or Crazing
        - Causes: Heat damage, chemical exposure, or manufacturing defects
        - Identification: Rainbow-like appearance or visible peeling
        - Warranty Check: Verify manufacturer's warranty coverage
        - Solution: Complete lens replacement is usually required

        3. Smudging and Cleaning Issues
        - Cause: Oil-based makeup, lotion, or improper cleaning
        - Solution: Demonstrate proper cleaning technique (warm water, soap, microfiber)
        - Products to Avoid: Harsh cleaners, paper products, clothing
        - Recommendation: Hydrophobic treatments for easier maintenance

        4. Fogging
        - Common Situations: Temperature changes, mask wearing
        - Solutions: Anti-fog treatments and cloths
        - Application Technique: Proper anti-fog application methods
        - Frame Adjustments: How frame positioning can reduce fogging
      `,keyPoints:["Proper cleaning and care significantly extends lens life","Many surface issues can be prevented with patient education","Document warranty coverage and expiration for all dispensed eyewear"]}],practiceQuestions:[{question:"What is the most likely cause of new glasses sliding down the nose?",options:["Frame too large","Nose pads too wide","Prescription too strong","Lenses too thick"],correctAnswer:1},{question:"A patient with new progressive lenses complains of peripheral distortion. What should you check first?",options:["Prescription accuracy","Frame adjustment","Pupillary distance","Lens material"],correctAnswer:1},{question:"Which contact lens issue requires immediate medical referral?",options:["Mild discomfort","Deposits on the lens","Red eye with pain","Vision fluctuation"],correctAnswer:2},{question:"What is the best solution for coating that is peeling off lenses?",options:["Clean with alcohol","Polish the surface","Apply new AR coating","Replace the lenses"],correctAnswer:3},{question:"How should you properly fix glasses with a loose screw?",options:["Apply glue to the screw","Replace with any available screw","Replace with correct size and apply screw lock","Tighten as firmly as possible"],correctAnswer:2}],relatedTopics:["frame-materials","lens-coatings","contact-lens-care"]},E={title:"Advanced Troubleshooting Techniques",description:"Advanced diagnostic approaches for challenging eyewear and vision issues",sections:[{title:"Complex Prescription Troubleshooting",content:`
        Addressing issues with high-powered or complex prescriptions:

        1. High Astigmatism Adaptation Problems
        - Symptoms: Spatial distortion, difficulty with stairs or depth perception
        - Testing Protocol: Check axis orientation and cylinder power accuracy
        - Adaptation Strategy: Consider temporary reduction in cylinder power
        - Progressive Integration: Special considerations for progressive addition with high cylinder

        2. High Anisometropia Challenges
        - Symptoms: Image size difference, difficulty with binocular fusion
        - Optical Solution: Prescribe with aniseikonic consideration 
        - Frame Selection: Choose frame with minimal pantoscopic tilt
        - Adaptation Protocol: Structured wearing schedule with gradual increase

        3. Prismatic Effects in High Prescriptions
        - Calculation Method: Calculate induced prism at reading position
        - Compensation Technique: Incorporate compensating prism
        - Frame Adjustment: Special considerations for frame adjustment with high prism
        - Troubleshooting Signs: Symptom pattern indicating unintended prismatic effect

        4. Multifocal Design Selection for Complex Cases
        - Analysis Process: When to choose specialized PAL designs
        - Special Cases: Post-cataract, keratoconus, and other irregular corneas
        - Back-vertex Power Considerations: Adjustments needed for high-plus lenses
        - Decision Matrix: Selecting between PAL, office lenses, or multiple pairs
      `,keyPoints:["Advanced prescriptions often require specialized lens designs and materials","Frame selection and adjustment are critical factors in complex prescriptions","Systematic documentation of adaptation progress helps refine future prescriptions"]},{title:"Specialty Contact Lens Troubleshooting",content:`
        Advanced approaches for specialty contact lens complications:

        1. Scleral Lens Fitting Issues
        - Evaluation Method: Assess clearance with OCT or slit lamp
        - Common Problems: Bubble formation, excessive settling, limbal bearing
        - Adjustment Protocol: Systematic vault and landing zone modification
        - Edge Profile Customization: When and how to modify edge design

        2. Rigid Gas Permeable (RGP) Lens Complications
        - Fluorescein Pattern Analysis: Interpreting specific pattern abnormalities
        - Topography Integration: Using corneal topography to refine fit
        - 3-9 Staining Management: Addressing common horizontal staining
        - Material Selection Strategy: Choosing materials based on specific patient factors

        3. Hybrid Lens Challenges
        - Junction Zone Assessment: Evaluating soft skirt/rigid center junction
        - Common Complications: Tear and separation issues
        - Modification Approach: When modification is possible vs. refit required
        - Patient Selection Criteria: Identifying ideal candidates to prevent issues

        4. Specialty Soft Lens Troubleshooting
        - Toric Rotation Issues: Systematic approach to rotation stabilization
        - Multifocal Vision Problems: Power adjustment strategy based on specific complaints
        - Custom Parameter Refinement: Process for refining custom parameters
        - Material Compatibility: Addressing solution sensitivity with specialty materials
      `,keyPoints:["Specialty contact lenses require systematic and methodical troubleshooting","Documentation with images or video improves tracking of fitting changes","Many specialty lens issues require incremental rather than dramatic changes"]},{title:"Advanced Optical Troubleshooting",content:`
        Diagnosing and solving complex optical challenges:

        1. Wavefront-Guided Analysis
        - Application: Using wavefront analysis to identify higher-order aberrations
        - Prescription Refinement: Incorporating wavefront data into prescriptions
        - Special Lens Designs: Free-form lenses based on wavefront data
        - Diagnostic Indicators: Symptoms suggesting higher-order aberration issues

        2. Vertex Distance Calculations
        - Critical Cases: When vertex distance adjustment is necessary
        - Calculation Protocol: Formula and method for accurate power adjustment
        - Frame Selection Impact: How frame selection affects effective power
        - Special Considerations: High myopia vs. high hyperopia adjustments

        3. Pantoscopic Tilt Effects
        - Optical Consequences: How tilt affects effective cylinder power and axis
        - Measurement Technique: Accurate measurement of frame tilt
        - Compensation Method: Adjusting prescription for non-standard tilt
        - Face Form Wrap Considerations: Combined effects of wrap and tilt

        4. Digital Eye Strain Advanced Solutions
        - Detailed Assessment: Protocol for digital usage pattern evaluation
        - Specialized Testing: Contrast sensitivity and accommodative facility
        - Advanced Lens Options: Blue light, anti-fatigue, and office lens designs
        - Environmental Modification: Advanced workspace ergonomic assessment
      `,keyPoints:["Advanced optical issues often require specialized measuring equipment","Small measurement errors can significantly impact complex prescriptions","Specialized lenses may require different fitting and adjustment protocols"]},{title:"Pediatric Vision Troubleshooting",content:`
        Special considerations for troubleshooting children's eyewear:

        1. Amblyopia Therapy Compliance
        - Assessment Method: Evaluate patching or atropine compliance
        - Problem-Solving Approach: Creative solutions for resistant children
        - Monitoring Protocol: Structured follow-up and progress tracking
        - Parent Education: Advanced training for monitoring and encouragement

        2. Myopia Management Complications
        - Treatment Efficacy Assessment: Evaluating progression despite intervention
        - Multifocal Contact Lens Issues: Special troubleshooting for pediatric wear
        - Orthokeratology Complications: Identifying and addressing adverse events
        - Atropine Side Effect Management: Strategies for light sensitivity and near blur

        3. Pediatric Frame Adjustment Challenges
        - Growth Accommodation: Adjusting frames for rapid growth
        - Sports Compatibility: Ensuring stability during physical activity
        - Special Needs Considerations: Adaptations for sensory sensitivity
        - Repair Frequency Reduction: Reinforcement techniques for durability

        4. Binocular Vision Therapy Interface
        - Prescription Timing: Coordination with vision therapy milestones
        - Prism Adaptation: Monitoring and adjusting prescribed prism
        - Lens Power Changes: When and how to implement power changes during therapy
        - Coordination Protocol: Communication system between therapist and optician
      `,keyPoints:["Pediatric troubleshooting requires developmental considerations","Frequent follow-up is essential for rapidly changing pediatric needs","Parent/caregiver education is critical for successful troubleshooting"]},{title:"Post-Surgical Vision Correction",content:`
        Addressing special needs after eye surgery:

        1. Post-LASIK/PRK Complications
        - Assessment Protocol: Evaluating residual refractive error
        - Dry Eye Management: Specialized lens options for persistent dry eye
        - Night Vision Issues: Prescription strategies for reduced contrast sensitivity
        - Timing Guidelines: When to finalize prescription post-surgery

        2. Post-Cataract Vision Correction
        - Multifocal IOL Enhancement: Glasses strategies for multifocal IOL gaps
        - Monofocal IOL Solutions: Prescribing approach for near vision needs
        - Unexpected Outcomes: Addressing unanticipated refractive results
        - Adaptation Timeline: Expected adaptation periods and intervention points

        3. Post-Corneal Transplant Challenges
        - Irregularity Management: Specialty lens options for irregular astigmatism
        - Progressive Prescription Strategy: Approach during healing period
        - Complication Indicators: Recognizing signs requiring medical referral
        - Material Selection: Considerations for oxygen permeability and wettability

        4. Refractive Surgery Enhancement Interface
        - Temporary Correction Options: Solutions while awaiting enhancement
        - Measurement Protocol: Special considerations for accurate measurements
        - Communication Framework: Coordinating with surgical team
        - Documentation Requirements: Critical measurements and observations
      `,keyPoints:["Post-surgical correction requires coordination with the surgical team","Prescription stability timing varies by procedure type and individual healing","Specialty contact lenses are often required for optimal post-surgical correction"]}],practiceQuestions:[{question:"Which lens design consideration is most important for a patient with high anisometropia?",options:["Center thickness","Aniseikonic design","Material density","Base curve"],correctAnswer:1},{question:"What is the primary concern when fitting scleral lenses?",options:["Lens diameter","Corneal clearance","Edge profile","Center thickness"],correctAnswer:1},{question:"When should vertex distance calculations be applied to a prescription?",options:["For all prescriptions","For prescriptions over ±4.00D","Only for multifocals","Only for astigmatism"],correctAnswer:1},{question:"Which approach is most appropriate for a child struggling with amblyopia treatment compliance?",options:["Increase patching hours","Switch to atropine","Create a reward system","Discontinue treatment temporarily"],correctAnswer:2},{question:"After cataract surgery with monofocal IOL, what is the most common vision correction need?",options:["Distance vision","Intermediate vision","Near vision","Astigmatism correction"],correctAnswer:2}],relatedTopics:["high-prescription-management","specialty-contact-lenses","surgical-co-management"]},b={commonIssuesGuide:M,advancedTroubleshooting:E},B=()=>{var m;const[d,C]=u.useState(0),[A,p]=u.useState("section0"),P=(i,t)=>{C(t),p("section0")},x=i=>(t,s)=>{p(s?i:!1)},r=d===0?b.commonIssuesGuide:b.advancedTroubleshooting;return e.jsx(o,{sx:{minHeight:"100vh",py:4},children:e.jsx(S,{maxwidth:!1,children:e.jsxs(h,{sx:{p:4,borderRadius:2},children:[e.jsx(n,{variant:"h4",component:"h1",gutterBottom:!0,children:"Troubleshooting Guide"}),e.jsx(n,{paragraph:!0,children:"Learn how to diagnose and solve common eyewear issues with these comprehensive guides."}),e.jsx(o,{sx:{borderBottom:1,borderColor:"divider",mb:3},children:e.jsxs(D,{value:d,onChange:P,"aria-label":"troubleshooting content tabs",children:[e.jsx(v,{label:"Common Issues"}),e.jsx(v,{label:"Advanced Troubleshooting"})]})}),e.jsxs(o,{sx:{mb:4},children:[e.jsx(n,{variant:"h5",gutterBottom:!0,children:r.title}),e.jsx(n,{variant:"body1",paragraph:!0,color:"text.secondary",children:r.description}),r.sections.map((i,t)=>e.jsxs(T,{expanded:A===`section${t}`,onChange:x(`section${t}`),sx:{mb:2},children:[e.jsx(k,{expandIcon:e.jsx(q,{}),"aria-controls":`section${t}-content`,id:`section${t}-header`,children:e.jsx(n,{variant:"h6",children:i.title})}),e.jsxs(I,{children:[e.jsx(n,{component:"div",sx:{whiteSpace:"pre-line",mb:3},children:i.content}),e.jsxs(o,{sx:{mt:2,bgcolor:"background.default",p:2,borderRadius:1},children:[e.jsx(n,{variant:"subtitle1",fontWeight:"bold",gutterBottom:!0,children:"Key Points:"}),e.jsx(g,{dense:!0,children:i.keyPoints.map((s,a)=>e.jsx(f,{sx:{py:.5},children:e.jsx(y,{primary:s})},a))})]})]})]},`section${t}`))]}),r.practiceQuestions.length>0&&e.jsxs(o,{sx:{my:4},children:[e.jsx(n,{variant:"h5",gutterBottom:!0,children:"Practice Questions"}),e.jsx(j,{sx:{mb:3}}),e.jsx(w,{container:!0,spacing:3,children:r.practiceQuestions.map((i,t)=>e.jsx(w,{item:!0,xs:12,md:6,children:e.jsxs(h,{elevation:1,sx:{p:2,height:"100%",border:"1px solid",borderColor:"divider"},children:[e.jsxs(n,{variant:"subtitle1",fontWeight:"bold",gutterBottom:!0,children:["Question ",t+1,": ",i.question]}),e.jsx(g,{dense:!0,children:i.options.map((s,a)=>e.jsx(f,{children:e.jsx(y,{primary:`${String.fromCharCode(65+a)}. ${s}`,sx:{"& .MuiTypography-root":{fontWeight:i.correctAnswer===a?"bold":"normal",color:i.correctAnswer===a?"primary.main":"inherit"}}})},a))})]})},t))})]}),((m=r.relatedTopics)==null?void 0:m.length)>0&&e.jsxs(o,{sx:{mt:4},children:[e.jsx(n,{variant:"subtitle1",fontWeight:"bold",children:"Related Topics:"}),e.jsx(o,{sx:{display:"flex",flexWrap:"wrap",gap:1,mt:1},children:r.relatedTopics.map((i,t)=>e.jsx(c,{component:l,to:`/topics/${i}`,variant:"outlined",size:"small",sx:{textTransform:"none"},children:i.split("-").map(s=>s.charAt(0).toUpperCase()+s.slice(1)).join(" ")},t))})]}),e.jsxs(o,{sx:{mt:4,display:"flex",justifyContent:"space-between"},children:[e.jsx(c,{component:l,to:"/",variant:"outlined",children:"Back to Home"}),e.jsx(c,{component:l,to:"/study",variant:"contained",color:"primary",children:"Go to Study Materials"})]})]})})})};export{B as default};
